...
class PackageController extends Controller
{
	/**
	 * index方法：套餐页接口
	 * 该方法根据packageId获取指定套餐内容 一次访问只返回一个套餐数据
	 */
	...
	public function getAll(){
		...
		$package = M('Package');
		$packageCondition['enable'] = 1;
		$packageList = $package->where($packageCondition)->order('id')->select();
		...
		elseif($packageList == null){
			...
			$data = array();
			foreach($packageList as $p){
				$imagePathGroup = explode('|', $p['image_path_group']);
				$imagePathGroup2 = array();
				foreach($imagePathGroup as $path){
					array_push($imagePathGroup2, getCompleteAddress($path));
				}
				$tmp = array(
						...
						'alias' => html_entity_decode($p['alias'], ENT_QUOTES, 'UTF-8'),
						'price' => html_entity_decode($p['price'], ENT_QUOTES, 'UTF-8'),
						'content' => html_entity_decode($p['content'], ENT_QUOTES, 'UTF-8'),
						'describe' => html_entity_decode($p['describe'], ENT_QUOTES, 'UTF-8'),
						'cover_path' => getCompleteAddress($p['cover_path']),
						'image_path_group' => $imagePathGroup2
				);
				array_push($data, $tmp);
			}
