...
class IndexController extends Controller
{
	...
	public function init(){
		...
		//版本更新
		$clientVersion = floatval(I('get.version'));//获取客户端版本号
		$newestVersion = floatval(C('NEWEST_VERSION')); //获取当前最新版本
		echo $clientVersion.'**'.$newestVersion;
		if($newestVersion > $clientVersion){
			$upt = array(
					'if_update' => 1,
					'update_url' => ''
			);
		...
		//获取base_url
		$versionRoot = C('BASE_URL_ARR')[$clientVersion];
